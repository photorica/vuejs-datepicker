  ...
  <div :class="[calendarClass, 'vdp-datepicker__calendar']" v-show="showDayView" :style="calendarStyle" @mousedown.prevent>
    ...
    <div :class="isRtl ? 'flex-rtl' : ''">
      ...
      <vx-tooltip
        ...
        v-for="day in days"
        :key="day.timestamp"
        :class="dayClasses(day, holidayDates)"
        :text="dayRokuyo(day, holidayDates)">
        <span
  ...
  methods: {
    ...
    dayClasses (day, holidayDates) {
      ...
      return {
        ...
        'public-holiday': holidayJp.default.isHoliday(targetDate),
    ...
    dayRokuyo (day, holidayDates) {
      const targetDate = new Date(day.timestamp)
      const isRegularHoliday = holidayDates.some((holidayDate) => {
        const date = new Date(holidayDate)
        return targetDate.getFullYear() === date.getFullYear() && targetDate.getMonth() === date.getMonth() && targetDate.getDate() === date.getDate()
      })
      return isRegularHoliday ? `休業日 ${targetDate.getRokuyo(1)}` : targetDate.getRokuyo(1)
    },
    /**
